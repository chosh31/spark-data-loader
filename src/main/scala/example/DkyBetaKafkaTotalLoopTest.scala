...
object DkyBetaKafkaTotalLoopTest {
  def main(args: Array[String]) {
    ...
    //val path = "/home/ec2-user/parquet/totalTest"
    ...
    for (topic <- topics) {
      configureStream(Array(topic), ssc, kafkaParams, sqlContext, defaultPath + "/" + topic)
    }
    ssc.start()
    ssc.awaitTermination()
  }

  def configureStream(topics: Array[String], ssc: StreamingContext, kafkaParams: Map[String, Object], sqlContext: SQLContext, path: String) {
